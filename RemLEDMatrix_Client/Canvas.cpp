...
//
void Canvas::update(CProgram &program) {
	for (int y = 0; y < _LED_matrix->size(); y++) {
		for (int x = 0; x < _LED_matrix->at(y)->size(); x++) {
			if (_LED_matrix->at(y)->at(x)->getGlobalBounds().contains(
				...
				if (program.lastButton == sf::Mouse::Button::Left)
					_LED_matrix->at(y)->at(x)->setFillColor(program.curColor->getFillColor());
				else if (program.lastButton == sf::Mouse::Button::Right)
					program.adjustColorSlider(_LED_matrix->at(y)->at(x)->getFillColor());
			}
		}
	}
}
